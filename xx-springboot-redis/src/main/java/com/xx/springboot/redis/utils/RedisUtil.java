package com.xx.springboot.redis.utils;
...
public class RedisUtil {
     ...
     * expire key seconds
     * @param key redis key
     * @param timeout 时间单位数量
     * @param unit 时间单位
     * @return 操作结果，成功true,失败false
     */
     ...
     * move key db
     * @param key  redis key
     * @param dbIndex 数据库序号，redis默认16个库 0-15
     * @return 操作结果，成功true,失败false
     */
     ...
     * 修改 key 的名称
     * rename key newkey
     * @param oldKey 旧key的名称
     * @param newKey 新key的名称
     */
     ...
     * type key
     * @param key redis key
     * @return 对于大多数的redis数据类型 使用这个命令都会返回String, 如list/set/zset/hash都会返回String
     */
     ...
     * 前缀每次扫描的记录数
     */
    private static Integer scanCount = 1000;
     ...
     * @return
     */
    public static List<String> scan(String prefixKey) {
        List<String> result = new ArrayList<>();
        try {
            ScanOptions options = ScanOptions.scanOptions()
                    .count(scanCount)
                    .match(prefixKey + "*").build();
            ...
            log.error("redis 前缀匹配[{}]出现异常",prefixKey,ex);
            return null;
        }
     ...
     * @return
     */
    public static boolean setAdd(String key, Object value) {
        try {
            redisTemplate.opsForSet().add(key, value);
            return true;
        } catch (Exception e) {
            log.error("setAdd() is error : {}", e);
            return false;
        }
    }
    ...
    public static Set<Object> getSetMembers(String key) {
        try {
            ...
            log.error("getSetMembers() is error : {}", e);
            return null;
        }
    }
    ...
    public static <T> List<T> getList(String key, Class<T> clazz) {
        try {
            ...
            if (null != cache && StringUtils.hasLength(cache.toString())) {
                List<T> cacheList = JSONArray.parseArray(JSON.toJSONString(cache), clazz);
                return cacheList;
            }
        } catch (Exception e) {
    ...
    public static <T> void setList(String key, List<T> list, long time) {
        if (!StringUtils.hasLength(key) || CollectionUtils.isEmpty(list)) {
            return;
        }
